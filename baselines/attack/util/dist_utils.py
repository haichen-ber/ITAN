...
class ChamferkNNDhusdorfist(nn.Module):

    def __init__(self, chamfer_method='adv2ori',
                 knn_k=5, knn_alpha=1.05,
                 chamfer_weight=1., knn_weight=1., hausdor_weight=1.0):
        """Geometry-aware distance function of AAAI'20 paper.
        ...
        Args:
            ...
            knn_weight (float, optional): weight factor. Defaults to 3..
        """
        super(ChamferkNNDhusdorfist, self).__init__()

        self.chamfer_dist = ChamferDist(method=chamfer_method)
        self.knn_dist = KNNDist(k=knn_k, alpha=knn_alpha)
        self.hausdor_dist = HausdorffDist(method=chamfer_method)
        self.w1 = chamfer_weight
        self.w2 = knn_weight
        self.w3 = hausdor_weight
    ...
    def forward(self, adv_pc, ori_pc,
        ...
        chamfer_loss = self.chamfer_dist(
            adv_pc, ori_pc, weights=weights, batch_avg=batch_avg)
        hausdorff_loss = self.hausdor_dist(adv_pc, ori_pc, weights=weights, batch_avg=batch_avg)
        knn_loss = self.knn_dist(
            adv_pc, weights=weights, batch_avg=batch_avg)
        loss = chamfer_loss * self.w1 + knn_loss * self.w2 + self.w3 * hausdorff_loss
        return loss
...
class ChamferL2(nn.Module):
    ...
    def forward(self, adv_pc, ori_pc,
        ...
        chamfer_loss = self.chamfer_dist(
            adv_pc, ori_pc, weights=weights, batch_avg=batch_avg)
        l2_loss = self.l2_dist(adv_pc, ori_pc, weights=weights, batch_avg=batch_avg)
      
        loss = chamfer_loss * self.w1 + l2_loss * self.w2
        return loss


class Chamferhusdorfist(nn.Module):

    def __init__(self, chamfer_method='adv2ori',
                 chamfer_weight=1., hausdor_weight=1.0):
        """Geometry-aware distance function of AAAI'20 paper.
        ...
        Args:
            ...
            knn_weight (float, optional): weight factor. Defaults to 3..
        """
        super(Chamferhusdorfist, self).__init__()
    ...
    def forward(self, adv_pc, ori_pc,
        ...
        loss = chamfer_loss * self.w1 + self.w3 * hausdorff_loss
        return loss