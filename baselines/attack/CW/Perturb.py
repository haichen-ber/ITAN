...
class CWPerturb:
    ...
    def __init__(self, model_name, model, adv_func, dist_func, attack_lr=1e-2,
    ...
    def attack(self, data, noise, target):
        ...
        if self.model_name !=  'pointtransformer':
            o_bestattack = torch.ones(B, 3, K).to(data.device)
        else:
            o_bestattack = torch.ones(B, K, 3).to(data.device)
        ...
        return o_bestattack, success_num
