...
class PointNetCls(nn.Module):
    ...
    def forward(self, x):
        feature, trans, trans_feat = self.feat(x) #返回这个特征 torch.Size([1, 1024])
        feature1 = self.relu(self.fc1(feature)) #torch.Size([1, 512])
        feature2 = self.dropout(self.fc2(feature1)) #torch.Size([1, 256])
        if self.use_bn:
            x = self.bn2(feature2)
        feature3 = self.relu(x)
        x = self.fc3(feature3)
        return x, trans, trans_feat, [feature, feature1, feature3]
