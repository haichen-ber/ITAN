...
def download():
    DATA_DIR = 'baselines/official_data'
    if not os.path.exists(DATA_DIR):
        os.mkdir(DATA_DIR)
    if not os.path.exists(os.path.join(DATA_DIR, 'scanobject')):
...
def load_scanobjectnn_data(partition):
    download()
    BASE_DIR = 'baselines/official_data/'
    all_data = []
    all_label = []

    h5_name = BASE_DIR + 'scanobject/main_split/' + partition + '_objectdataset_augmentedrot_scale75.h5'
